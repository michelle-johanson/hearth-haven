...
const defaultCooperationLevels = [
  'Highly Cooperative',
  'Cooperative',
  'Neutral',
  'Uncooperative',
];
const defaultVisitOutcomes = [
  'Favorable',
  'Needs Improvement',
  'Unfavorable',
  'Inconclusive',
...
function getEducationFields(opts?: GlobalEducationOptions): RecordFieldDef[] {
  return [
    { key: 'recordDate', label: 'Record Date', type: 'date', required: true },
    {
      ...
      type: 'select',
      options: mergeOpts(defaultEducationLevels, opts?.educationLevels),
      required: true,
    },
    { key: 'schoolName', label: 'School Name', type: 'text' },
    {
      ...
      type: 'select',
      options: mergeOpts([], opts?.enrollmentStatuses),
      required: true,
    },
    {
      key: 'attendanceRate',
    ...
    },
    {
      ...
      type: 'select',
      options: mergeOpts(defaultCompletionStatuses, opts?.completionStatuses),
      required: true,
    },
    { key: 'notes', label: 'Notes', type: 'textarea' },
  ];
...
function getIncidentFields(
  ...
  return [
    ...
    },
    {
      ...
      label: 'Incident Type',
      type: 'select',
      options: mergeOpts(defaultIncidentTypes, opts?.incidentTypes),
      required: true,
    },
    {
      key: 'severity',
      label: 'Severity',
      type: 'select',
      options: mergeOpts(defaultSeverities, opts?.severities),
      required: true,
    },
...
function getVisitationFields(
  opts?: GlobalVisitationOptions,
  socialWorkers?: string[]
): RecordFieldDef[] {
  return [
    { key: 'visitDate', label: 'Visit Date', type: 'date', required: true },
    {
      ...
      label: 'Social Worker',
      type: 'select',
      options: socialWorkers ?? opts?.socialWorkers ?? [],
      required: true,
    },
    {
      key: 'visitType',
      label: 'Visit Type',
      type: 'select',
      options: mergeOpts(defaultVisitTypes, opts?.visitTypes),
      required: true,
    },
    { key: 'locationVisited', label: 'Location Visited', type: 'text' },
    {
      key: 'familyMembersPresent',
    ...
    { key: 'observations', label: 'Observations', type: 'textarea' },
    {
      ...
      type: 'select',
      options: mergeOpts(defaultCooperationLevels, opts?.cooperationLevels),
      required: true,
    },
    ...
    { key: 'followUpNotes', label: 'Follow-up Notes', type: 'textarea' },
    {
      ...
      label: 'Visit Outcome',
      type: 'select',
      options: mergeOpts(defaultVisitOutcomes, opts?.visitOutcomes),
      required: true,
    },
  ];
}

function getProcessRecordingFields(
  opts?: GlobalProcessRecordingOptions,
  socialWorkers?: string[]
): RecordFieldDef[] {
  return [
    ...
    },
    {
      ...
      label: 'Session Type',
      type: 'select',
      options: mergeOpts(defaultSessionTypes, opts?.sessionTypes),
      required: true,
    },
    {
    ...
    },
    {
      ...
      type: 'select',
      options: mergeOpts(defaultEmotionalStates, opts?.emotionalStates),
      required: true,
    },
    {
      key: 'emotionalStateEnd',
...
export default function ResidentDetailPage() {
  ...
  const [healthTotal, setHealthTotal] = useState({
    totalPages: 1,
    totalCount: 0,
  });
  ...
  const [eduFilterOpts, setEduFilterOpts] =
    useState<EducationFilterOptions | null>(null);
  ...
  const [incFilterOpts, setIncFilterOpts] =
  ...
  const [visFilterOpts, setVisFilterOpts] =
    useState<VisitationFilterOptions | null>(null);
  ...
  const [procFilterOpts, setProcFilterOpts] =
    useState<ProcessRecordingFilterOptions | null>(null);
  ...
  const [globalProcOpts, setGlobalProcOpts] =
    useState<GlobalProcessRecordingOptions>();
  ...
  >([]);
  const [allEducationRecords, setAllEducationRecords] = useState<
    EducationRecord[]
  >([]);
  ...
  useEffect(() => {
    ...
    fetchInterventionPlans(residentId)
      .then(setInterventionPlans)
      .catch(console.error);
  ...
  useEffect(() => {
    ...
    fetchIncidentReports(residentId, incPage, 10, incFilters)
      .then((res) => {
        ...
        setIncTotal({ totalPages: res.totalPages, totalCount: res.totalCount });
      })
      .catch(console.error);
  ...
  useEffect(() => {
    ...
    fetchVisitations(residentId, visPage, 10, visFilters)
      .then((res) => {
        setVisitations(res.data);
        setVisTotal({ totalPages: res.totalPages, totalCount: res.totalCount });
      })
      .catch(console.error);
  ...
  useEffect(() => {
    ...
    fetchEducationRecords(residentId, eduPage, 10, eduFilters)
      .then((res) => {
        ...
        setEduTotal({ totalPages: res.totalPages, totalCount: res.totalCount });
      })
      .catch(console.error);
  ...
  useEffect(() => {
    if (activeTab === 'education' && !eduFilterOpts)
      fetchEducationFilterOptions(residentId)
        .then(setEduFilterOpts)
        .catch(console.error);
    if (activeTab === 'safety' && !incFilterOpts)
      fetchIncidentFilterOptions(residentId)
        .then(setIncFilterOpts)
        .catch(console.error);
    if (activeTab === 'safety' && !visFilterOpts)
      fetchVisitationFilterOptions(residentId)
        .then(setVisFilterOpts)
        .catch(console.error);
    if (activeTab === 'physicalHealth' && !procFilterOpts)
      fetchProcessRecordingFilterOptions(residentId)
        .then(setProcFilterOpts)
        .catch(console.error);
  }, [activeTab]);
  ...
  useEffect(() => {
    if (activeTab === 'safety')
      fetchIncidentReports(residentId, 1, 500, {})
        .then((res) => setAllIncidents(res.data))
        .catch(console.error);
    if (activeTab === 'physicalHealth')
      fetchHealthRecords(residentId, 1, 500, {})
        .then((res) => setAllHealthRecords(res.data))
        .catch(console.error);
    if (activeTab === 'education')
      fetchEducationRecords(residentId, 1, 500, {})
        .then((res) => setAllEducationRecords(res.data))
        .catch(console.error);
  }, [activeTab, residentId, refreshKey]);
  ...
  const handleRecordField = (key: string, value: unknown) => {
    ...
    setRecordModal({
      ...recordModal,
      data: { ...recordModal.data, [key]: value },
    });
  };
        ...
        switch (recordModal.entity) {
          case 'health':
            await createHealthRecord(
              recordModal.data as Partial<HealthWellbeingRecord>
            );
            break;
          case 'education':
            await createEducationRecord(
              recordModal.data as Partial<EducationRecord>
            );
            break;
          case 'incidents':
            await createIncidentReport(
              recordModal.data as Partial<IncidentReport>
            );
            break;
          case 'visitations':
            await createVisitation(recordModal.data as Partial<HomeVisitation>);
            break;
          case 'processRecordings':
            await createProcessRecording(
              recordModal.data as Partial<ProcessRecording>
            );
            break;
          case 'interventionPlan':
            await createInterventionPlan(
              recordModal.data as Partial<InterventionPlan>
            );
            break;
        }
      } else {
        ...
        switch (recordModal.entity) {
          case 'health':
            await updateHealthRecord(
              d.healthRecordId as number,
              d as unknown as HealthWellbeingRecord
            );
            break;
          case 'education':
            await updateEducationRecord(
              d.educationRecordId as number,
              d as unknown as EducationRecord
            );
            break;
          case 'incidents':
            await updateIncidentReport(
              d.incidentId as number,
              d as unknown as IncidentReport
            );
            break;
          case 'visitations':
            await updateVisitation(
              d.visitationId as number,
              d as unknown as HomeVisitation
            );
            break;
          case 'processRecordings':
            await updateProcessRecording(
              d.recordingId as number,
              d as unknown as ProcessRecording
            );
            break;
          case 'interventionPlan':
            await updateInterventionPlan(
              d.planId as number,
              d as unknown as InterventionPlan
            );
            break;
        }
      }
  ...
  const handleRecordDelete = async () => {
    ...
    try {
      ...
      switch (recordModal.entity) {
        case 'health':
          await deleteHealthRecord(d.healthRecordId as number);
          break;
        case 'education':
          await deleteEducationRecord(d.educationRecordId as number);
          break;
        case 'incidents':
          await deleteIncidentReport(d.incidentId as number);
          break;
        case 'visitations':
          await deleteVisitation(d.visitationId as number);
          break;
        case 'processRecordings':
          await deleteProcessRecording(d.recordingId as number);
          break;
        case 'interventionPlan':
          await deleteInterventionPlan(d.planId as number);
          break;
      }
      setRecordModal(null);
      triggerRefresh();
    ...
    if (col.key === 'safehouseId')
      return (
        <select
          ...
          onChange={(e) => handleEditField(col.key, Number(e.target.value))}
        >
          {safehouses.map((sh) => (
    ...
    if (sc && filterOptions) {
      ...
      return (
        <select
          ...
          onChange={(e) => handleEditField(col.key, e.target.value || null)}
        >
  ...
  const DateRange = ({
    ...
    from?: string;
    to?: string;
    onChange: (f: string | undefined, t: string | undefined) => void;
  }) => (
    <>
  ...
  const [showPlanDeleteConfirm, setShowPlanDeleteConfirm] = useState<
    number | null
  >(null);
  ...
  const savePlan = async () => {
    ...
    try {
      await updateInterventionPlan(
        editingPlanId,
        editPlanData as unknown as InterventionPlan
      );
      setEditingPlanId(null);
      triggerRefresh();
    } catch (err) {
  ...
  const renderPlanField = (
    ...
    if (!isEditing)
      return (
        <div className="resident-modal-field">
          <label>{label}</label>
          <span className="resident-modal-field-value">{fmt(val)}</span>
        </div>
      );
    const onChange = (v: unknown) =>
      setEditPlanData((d) => ({ ...d, [key]: v }));
    let input;
    if (type === 'select' && options) {
      input = (
        <select
          className="ip-edit-field"
  ...
  const renderInterventionPlans = (categories: string[]) => {
    const plans = interventionPlans.filter((p) =>
      categories.includes(p.planCategory)
    );
      ...
      <div className="ip-section">
          ...
          <div className="ip-add-btns">
            {missingCategories.map((cat) => (
              <button
                ...
                onClick={() =>
                  openRecordCreate('interventionPlan', {
                    planCategory: cat,
                    status: 'Open',
        ...
        {plans.length === 0 ? (
          <p style={{ color: '#9ca3af', fontSize: 13 }}>
            No intervention plans for this category yet.
          </p>
        ) : (
          plans.map((p) => {
            const isEditing = editingPlanId === p.planId;
                ...
                <div className="ip-card-title-row">
                  ...
                  <div className="ip-card-actions">
                    {isEditing ? (
                      <>
                        <button
                          className="ip-icon-btn ip-icon-save"
                          onClick={savePlan}
                          disabled={planSaving}
                          title="Save"
                        >
                        ...
                        <button
                          className="ip-icon-btn ip-icon-cancel"
                      ...
                      <>
                        <button
                          className="ip-icon-btn ip-icon-edit"
                        ...
                        <button
                          className="ip-icon-btn ip-icon-delete"
                          onClick={() => setShowPlanDeleteConfirm(p.planId)}
                          title="Delete"
                        >
                          <svg
                            width="15"
        ...
        {showPlanDeleteConfirm != null &&
          createPortal(
            <div
              className="resident-modal-overlay"
              onClick={() => setShowPlanDeleteConfirm(null)}
            >
              <div
              ...
              >
                ...
                <div className="delete-confirm-actions">
                  <button
                    ...
                    onClick={() => confirmDeletePlan(showPlanDeleteConfirm)}
                    disabled={planSaving}
                  >
  ...
  const renderHealthTab = () => (
    <>
      <div className="tab-filter-bar">
        <DateRange
          from={healthFilters.dateFrom}
          to={healthFilters.dateTo}
          onChange={(f, t) => {
            setHealthFilters((p) => ({ ...p, dateFrom: f, dateTo: t }));
            setHealthPage(1);
          }}
        />
        <BoolSelect
          label="Medical"
          value={healthFilters.medicalCheckupDone}
          onChange={(v) => {
            setHealthFilters((p) => ({ ...p, medicalCheckupDone: v }));
            setHealthPage(1);
          }}
        />
        <BoolSelect
          label="Dental"
          value={healthFilters.dentalCheckupDone}
          onChange={(v) => {
            setHealthFilters((p) => ({ ...p, dentalCheckupDone: v }));
            setHealthPage(1);
          }}
        />
        <BoolSelect
          label="Psychological"
          value={healthFilters.psychologicalCheckupDone}
          onChange={(v) => {
            setHealthFilters((p) => ({ ...p, psychologicalCheckupDone: v }));
            setHealthPage(1);
          }}
        />
        <button
          className="resident-modal-btn resident-modal-btn-edit tab-add-btn"
          onClick={() =>
            openRecordCreate('health', {
      ...
      {healthRecords.length === 0 ? (
        <p className="tab-empty">No health & wellbeing records found.</p>
      ) : (
        <>
            ...
            <table className="case-table">
              ...
              <tbody>
                {healthRecords.map((r) => (
                  ...
                  >
                    <td>{r.recordDate}</td>
                    <td>{fmt(r.generalHealthScore)}</td>
                    <td>{fmt(r.nutritionScore)}</td>
                    <td>{fmt(r.sleepQualityScore)}</td>
                    <td>{fmt(r.energyLevelScore)}</td>
                    <td>{fmt(r.heightCm)}</td>
                    <td>{fmt(r.weightKg)}</td>
                    <td>{fmt(r.bmi)}</td>
                    <td>{fmt(r.medicalCheckupDone)}</td>
                    <td>{fmt(r.dentalCheckupDone)}</td>
                    <td>{fmt(r.psychologicalCheckupDone)}</td>
                    <td>{fmt(r.notes)}</td>
                  </tr>
                ))}
              </tbody>
          ...
          <TabPagination
            page={healthPage}
            totalPages={healthTotal.totalPages}
            totalCount={healthTotal.totalCount}
  ...
  const renderEducationTab = () => (
    <>
      <div className="tab-filter-bar">
        <DateRange
          from={eduFilters.dateFrom}
          to={eduFilters.dateTo}
          onChange={(f, t) => {
            setEduFilters((p) => ({ ...p, dateFrom: f, dateTo: t }));
            setEduPage(1);
          }}
        />
        {eduFilterOpts && (
          <>
            <select
              value={eduFilters.educationLevel || ''}
              onChange={(e) => {
                setEduFilters((p) => ({
                  ...p,
                  educationLevel: e.target.value || undefined,
                }));
                setEduPage(1);
              }}
            >
              <option value="">All Levels</option>
              {eduFilterOpts.educationLevels.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
            ...
            <select
              value={eduFilters.enrollmentStatus || ''}
              onChange={(e) => {
                setEduFilters((p) => ({
                  ...p,
                  enrollmentStatus: e.target.value || undefined,
                }));
                setEduPage(1);
              }}
            >
              <option value="">All Enrollment</option>
              {eduFilterOpts.enrollmentStatuses.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
            ...
            <select
              value={eduFilters.completionStatus || ''}
              onChange={(e) => {
                setEduFilters((p) => ({
                  ...p,
                  completionStatus: e.target.value || undefined,
                }));
                setEduPage(1);
              }}
            >
              <option value="">All Completion</option>
              {eduFilterOpts.completionStatuses.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
      ...
      {educationRecords.length === 0 ? (
        <p className="tab-empty">No education records found.</p>
      ) : (
        <>
          <div className="tab-table-wrap">
            <table className="case-table">
              ...
              <tbody>
                {educationRecords.map((r) => (
                  ...
                  >
                    ...
                    <td>{fmt(r.schoolName)}</td>
                    <td>{r.enrollmentStatus}</td>
                    <td>{(r.attendanceRate * 100).toFixed(1)}%</td>
                    <td>{r.progressPercent.toFixed(1)}%</td>
          ...
          <TabPagination
            page={eduPage}
            totalPages={eduTotal.totalPages}
            totalCount={eduTotal.totalCount}
  ...
  const renderIncidentsTab = () => (
    <>
      <div className="tab-filter-bar">
        <DateRange
          from={incFilters.dateFrom}
          to={incFilters.dateTo}
          onChange={(f, t) => {
            setIncFilters((p) => ({ ...p, dateFrom: f, dateTo: t }));
            setIncPage(1);
          }}
        />
        {incFilterOpts && (
          <>
            <select
              value={incFilters.incidentType || ''}
              onChange={(e) => {
                setIncFilters((p) => ({
                  ...p,
                  incidentType: e.target.value || undefined,
                }));
                setIncPage(1);
              }}
            >
              <option value="">All Types</option>
              {incFilterOpts.incidentTypes.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
            ...
            >
              ...
              {incFilterOpts.severities.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
        ...
        <BoolSelect
          label="Resolved"
          value={incFilters.resolved}
          onChange={(v) => {
            setIncFilters((p) => ({ ...p, resolved: v }));
            setIncPage(1);
          }}
        />
        <button
      ...
      {incidentReports.length === 0 ? (
        <p className="tab-empty">No incident reports found.</p>
      ) : (
        <>
          <div className="tab-table-wrap">
            <table className="case-table">
              ...
              <tbody>
                {incidentReports.map((r) => (
                  <tr
                    key={r.incidentId}
                    className={`case-row-clickable ${r.severity === 'High' ? 'incident-row-high' : ''}`}
                    onClick={() =>
                      openRecordView(
                        'incidents',
                        r as unknown as Record<string, unknown>
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`severity-badge severity-${r.severity?.toLowerCase()}`}
                      >
                        {r.severity}
                      </span>
                    </td>
                    <td>{fmt(r.description)}</td>
                    <td>{fmt(r.responseTaken)}</td>
                    <td>{fmt(r.resolved)}</td>
                    <td>{fmt(r.resolutionDate)}</td>
                    <td>{r.reportedBy}</td>
                    <td>{fmt(r.followUpRequired)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <TabPagination
            page={incPage}
            totalPages={incTotal.totalPages}
            totalCount={incTotal.totalCount}
  ...
  const renderVisitationsTab = () => (
    <>
      <div className="tab-filter-bar">
        <DateRange
          from={visFilters.dateFrom}
          to={visFilters.dateTo}
          onChange={(f, t) => {
            setVisFilters((p) => ({ ...p, dateFrom: f, dateTo: t }));
            setVisPage(1);
          }}
        />
        {visFilterOpts && (
          <>
            ...
            >
              <option value="">All Types</option>
              {visFilterOpts.visitTypes.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
            ...
            <select
              value={visFilters.familyCooperationLevel || ''}
              onChange={(e) => {
                setVisFilters((p) => ({
                  ...p,
                  familyCooperationLevel: e.target.value || undefined,
                }));
                setVisPage(1);
              }}
            >
              <option value="">All Cooperation</option>
              {visFilterOpts.cooperationLevels.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
            ...
            <select
              value={visFilters.socialWorker || ''}
              onChange={(e) => {
                setVisFilters((p) => ({
                  ...p,
                  socialWorker: e.target.value || undefined,
                }));
                setVisPage(1);
              }}
            >
              <option value="">All Workers</option>
              {visFilterOpts.socialWorkers.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
        ...
        <BoolSelect
          ...
          value={visFilters.safetyConcernsNoted}
          onChange={(v) => {
            setVisFilters((p) => ({ ...p, safetyConcernsNoted: v }));
            setVisPage(1);
          }}
        />
        <button
      ...
      {visitations.length === 0 ? (
        <p className="tab-empty">No visitation records found.</p>
      ) : (
        <>
          <div className="tab-table-wrap">
            <table className="case-table">
              ...
              <tbody>
                {visitations.map((r) => (
                  ...
                  >
                    ...
                    <td>{r.visitType}</td>
                    <td>{fmt(r.locationVisited)}</td>
                    <td>{fmt(r.purpose)}</td>
                    <td>{r.familyCooperationLevel}</td>
                    <td>{fmt(r.safetyConcernsNoted)}</td>
                    <td>{fmt(r.followUpNeeded)}</td>
                    <td>{r.visitOutcome}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <TabPagination
            page={visPage}
            totalPages={visTotal.totalPages}
            totalCount={visTotal.totalCount}
            onPageChange={setVisPage}
          />
        </>
      )}
    </>
  ...
  const renderProcessRecordingsTab = () => (
    <>
      <div className="tab-filter-bar">
        <DateRange
          from={procFilters.dateFrom}
          to={procFilters.dateTo}
          onChange={(f, t) => {
            setProcFilters((p) => ({ ...p, dateFrom: f, dateTo: t }));
            setProcPage(1);
          }}
        />
        {procFilterOpts && (
          <>
            ...
            >
              <option value="">All Types</option>
              {procFilterOpts.sessionTypes.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
            ...
            <select
              value={procFilters.socialWorker || ''}
              onChange={(e) => {
                setProcFilters((p) => ({
                  ...p,
                  socialWorker: e.target.value || undefined,
            ...
            >
              <option value="">All Workers</option>
              {procFilterOpts.socialWorkers.map((o) => (
                <option key={o} value={o}>
                  {o}
                </option>
              ))}
        ...
        <BoolSelect
          label="Concerns"
          value={procFilters.concernsFlagged}
          onChange={(v) => {
            setProcFilters((p) => ({ ...p, concernsFlagged: v }));
            setProcPage(1);
          }}
        />
        <button
          className="resident-modal-btn resident-modal-btn-edit tab-add-btn"
          onClick={() =>
            openRecordCreate('processRecordings', {
      ...
      {procRecordings.length === 0 ? (
        <p className="tab-empty">No counseling sessions found.</p>
      ) : (
        <>
          <div className="tab-table-wrap">
            <table className="case-table">
              ...
              <tbody>
                {procRecordings.map((r) => (
                  ...
                  >
                    ...
                    <td>{r.socialWorker}</td>
                    <td>{r.sessionType}</td>
                    <td>{r.sessionDurationMinutes}m</td>
                    <td>{r.emotionalStateObserved}</td>
                    <td>{r.emotionalStateEnd}</td>
                    <td>{fmt(r.progressNoted)}</td>
                    <td>{fmt(r.concernsFlagged)}</td>
                    <td>{fmt(r.referralMade)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <TabPagination
            page={procPage}
            totalPages={procTotal.totalPages}
            totalCount={procTotal.totalCount}
  ...
  const renderSafetyTab = () => {
    const plan =
      interventionPlans.find((p) => p.planCategory === 'Safety') ?? null;
    return (
      <>
  ...
  const renderPhysicalHealthTab = () => {
    const plan =
      interventionPlans.find((p) => p.planCategory === 'Physical Health') ??
      null;
    return (
      <>
  ...
  const renderEducationCompositeTab = () => {
    const plan =
      interventionPlans.find((p) => p.planCategory === 'Education') ?? null;
    return (
      <>
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'resident':
        return renderResidentTab();
      case 'safety':
        return renderSafetyTab();
      case 'physicalHealth':
        return renderPhysicalHealthTab();
      case 'education':
        return renderEducationCompositeTab();
    }
  };
  ...
  if (loading)
    return (
      <div className="resident-detail-page">
        <p className="case-status">Loading...</p>
      </div>
    );
  if (error)
    return (
      <div className="resident-detail-page">
        <p className="case-status case-error">Error: {error}</p>
      </div>
    );
  if (!resident || !editData)
    return (
      <div className="resident-detail-page">
        <p className="case-status">Resident not found.</p>
      </div>
    );
  ...
  return (
    <>
      <div className="resident-detail-page">
        <button
          className="resident-detail-back"
        ...
        <div className="resident-detail-card">
          <div className="resident-modal-top-bar">
            ...
            <div className="resident-modal-profile-info">
              ...
              <p>
                {resident.internalCode} &middot; {resident.caseStatus}
              </p>
            </div>
      ...
      {recordModal && (
        <RecordModal
          title={
            recordModal.entity === 'health'
              ? 'Health & Wellbeing Record'
              : recordModal.entity === 'education'
                ? 'Education Record'
                : recordModal.entity === 'incidents'
                  ? 'Incident Report'
                  : recordModal.entity === 'visitations'
                    ? 'Home Visitation'
                    : recordModal.entity === 'processRecordings'
                      ? 'Counseling Session'
                      : 'Intervention Plan'
          }
          fields={
            recordModal.entity === 'health'
              ? healthFields
              : recordModal.entity === 'education'
                ? getEducationFields(globalEduOpts)
                : recordModal.entity === 'incidents'
                  ? getIncidentFields(
                      globalIncOpts,
                      filterOptions?.socialWorkers
                    )
                  : recordModal.entity === 'visitations'
                    ? getVisitationFields(
                        globalVisOpts,
          ...
          onDelete={
            recordModal.mode === 'view' ? handleRecordDelete : undefined
          }
          onEdit={
        ...
        createPortal(
          ...
          >
            ...
            >
              ...
              <p>
                Are you sure you want to delete this resident record? This

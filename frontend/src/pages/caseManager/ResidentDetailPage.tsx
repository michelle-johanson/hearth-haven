...
import {
  fetchReintegrationPrediction,
  ReintegrationPrediction,
...
export default function ResidentDetailPage() {
  ...
  const [mlPrediction, setMlPrediction] =
    useState<ReintegrationPrediction | null>(null);
  const [progressPrediction, setProgressPrediction] =
    useState<ProgressPrediction | null>(null);
    ...
    fetchResident(residentId)
      .then((r) => {
        ...
        fetchReintegrationPrediction(residentId)
          ...
          .catch(() => {
            /* ML server unavailable — score card simply won't show */
          });
      ...
      <div className="mb-6">
        ...
        {plans.length === 0 ? (
          ...
          plans.map((p) => {
            ...
            return (
              <div
                className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 shadow-md mb-4"
                key={p.planId}
              >
                <div className="flex items-center gap-3 mb-3">
                  <strong className="flex-1 text-sm font-semibold text-gray-900 dark:text-white">
                    {p.planCategory}
                  </strong>
    ...
    <>
      ...
      {healthRecords.length === 0 ? (
        <p className="py-8 text-center text-sm text-gray-400">
          No health & wellbeing records found.
        </p>
      ) : (
        <>
  ...
  if (!resident || !editData)
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <p className="text-center text-sm text-gray-500">Resident not found.</p>
      </div>
    );
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-md">
          ...
          <div className="flex flex-wrap items-start gap-4 border-b border-gray-100 p-4 sm:items-center sm:p-6 dark:border-gray-700">
            <img
              ...
              alt="Resident"
              className="h-14 w-14 rounded-full border border-gray-200 dark:border-gray-700 object-cover"
            />

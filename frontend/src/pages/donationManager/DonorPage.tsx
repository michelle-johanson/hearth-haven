...
import { fetchDonorPrediction, DonorPrediction } from '../../api/donationManager/MLDonorAPI';
...
function TypeIcon({ type }: { type: string }) {
  switch (type) {
    case 'Monetary':
      return <DollarSign className="h-4 w-4 shrink-0 text-green-600" />;
    case 'InKind':
      return <Package className="h-4 w-4 shrink-0 text-blue-600" />;
    case 'Volunteer':
      return <HandHelping className="h-4 w-4 shrink-0 text-purple-600" />;
    case 'Skills':
      return <GraduationCap className="h-4 w-4 shrink-0 text-amber-600" />;
    case 'SocialMedia':
      return <Share2 className="h-4 w-4 shrink-0 text-pink-600" />;
    default:
      return <DollarSign className="h-4 w-4 shrink-0 text-gray-500" />;
  }
}
...
function SupporterIcon({ type }: { type: string }) {
  switch (type) {
    ...
    case 'PartnerOrganization':
      return <Building2 className="h-4 w-4 shrink-0 text-indigo-600" />;
    default:
      return <Users className="h-4 w-4 shrink-0 text-gray-500" />;
  }
}
...
export default function DonorPage() {
  ...
  >('supporters');
  const [pendingDonationId, setPendingDonationId] = useState<number | null>(
    null
  );
  const [pendingSupporterId, setPendingSupporterId] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    if (
      locationState?.tab === 'contributions' ||
      locationState?.tab === 'allocations'
    ) {
  ...
  const [donorPrediction, setDonorPrediction] =
    useState<DonorPrediction | null>(null);
  ...
  useEffect(() => {
    if (pendingDonationId && contributions.length > 0) {
      const match = contributions.find(
        (c) => c.donationId === pendingDonationId
      );
      if (match) {
  ...
  useEffect(() => {
    ...
    fetchSupporters(1, 1, { supporterId: pendingSupporterId })
      ...
      .catch((e) =>
        console.error('Failed to load supporter from navigation state:', e)
      )
      .finally(() => {
  ...
  const setSupporterTypeFilter = (t: string | undefined) => {
    if (t === 'Anonymous') {
      setSupporterFilters((p) => ({
        ...p,
        supporterId: ANONYMOUS_SUPPORTER_ID,
  ...
  const hasSupporterFilters = !!(
    supporterFilters.status ||
    supporterFilters.supporterType ||
    supporterFilters.supporterId ||
    supporterFilters.search
  );
  ...
  const hasContributionFilters = !!(
    ...
    contributionFilters.donationType ||
  ...
  const activeSupporterType =
    supporterFilters.supporterId === 62
      ? 'Anonymous'
      : supporterFilters.supporterType;
  ...
  return (
    <>
      {!isAuthenticated ? (
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          >
            ...
            <div
              className={`pb-6 ${sidebarCollapsed ? 'lg:px-2 px-5' : 'px-5'}`}
            >
          ...
          <div className="min-w-0 flex-1 p-4 sm:p-6 lg:p-8">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex w-full flex-wrap items-center gap-3 lg:w-auto lg:justify-end">
                <Link
                  to="/donor-analytics"
                  className="btn-secondary w-full no-underline sm:w-auto"
                >
                ...
                {activeTab === 'supporters' ? (
                  ...
                  <button
                    className="btn-primary w-full sm:w-auto"
                    onClick={() => allocationPageRef.current?.openCreate()}
                  >
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {activeTab === 'supporters' && (
                    <select
                      ...
                      onChange={(e) =>
                        setSupporterTypeFilter(e.target.value || undefined)
                      }
                      aria-label="Filter supporters by type"
                    >
                      <option value="">All Types</option>
                      <option value="Anonymous">Anonymous</option>
                      {(filterOptions?.supporterTypes ?? SUPPORTER_TYPES).map(
                        (t) => (
                  ...
                  {filterOptions && activeTab === 'contributions' && (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <select
                        ...
                        aria-label="Filter contributions by program area"
                      >
                      ...
                      <select
                        ...
                        aria-label="Filter contributions by safehouse"
                      >
            ...
            {/* ── ALLOCATIONS TAB ───────────────────────────────────────── */}
            {activeTab === 'allocations' && (
              <AllocationPage ref={allocationPageRef} />
            )}
      ...
      {selectedSupporter &&
        ...
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="supporter-dialog-title"
          ...
          >
            <div className="modal-body" onClick={(e) => e.stopPropagation()}>
              ...
              {donorPrediction &&
                (donorPrediction.lapse || donorPrediction.upgrade) && (
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <div className="rounded-lg bg-white dark:bg-gray-900 p-3 border border-gray-100 dark:border-gray-700">
                          ...
                          <div
                            className={`text-2xl font-bold mb-1 ${
                              donorPrediction.lapse.lapse_score >= 70
                                ? 'text-red-600 dark:text-red-400'
                                : donorPrediction.lapse.lapse_score >= 40
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            {donorPrediction.lapse.recommendation}
                          </div>
                        </div>
                      )}
                      {donorPrediction.upgrade && (
                        <div className="rounded-lg bg-white dark:bg-gray-900 p-3 border border-gray-100 dark:border-gray-700">
                          ...
                          <div
                            className={`text-2xl font-bold mb-1 ${
                              donorPrediction.upgrade.upgrade_score >= 60
                                ? 'text-green-600 dark:text-green-400'
                                : donorPrediction.upgrade.upgrade_score >= 35
                                  ? 'text-yellow-600 dark:text-yellow-400'
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            {donorPrediction.upgrade.recommendation}
                          </div>
                        </div>
                      )}
                    </div>
          ...
          >
            <div className="modal-body" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mb-6 flex items-start gap-4 border-b border-gray-100 pb-5 dark:border-gray-700">
                ...
                <div className="min-w-0 flex-1">
                  <h2
                    id="new-supporter-dialog-title"
                    className="text-lg font-bold text-gray-900 dark:text-white"
                  >
                    New Supporter
                  </h2>
                  <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                    Add a new supporter profile
                  </p>
                </div>
                <div className="ml-4 flex items-center gap-2">
                  <button
      ...
      {isAddingCon &&
        createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="new-contribution-dialog-title"
          ...
          >
            <div className="modal-body" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mb-6 flex items-start gap-4 border-b border-gray-100 pb-5 dark:border-gray-700">
                ...
                <div className="min-w-0 flex-1">
                  <h2
                    id="new-contribution-dialog-title"
                    className="text-lg font-bold text-gray-900 dark:text-white"
                  >
                    Record Contribution
                  </h2>
                  <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
                    Log a new donation or contribution
                  </p>
                </div>
                <div className="ml-4 flex items-center gap-2">
                  <button
      ...
      {showDeleteSupConfirm &&
        createPortal(
          <div
            ...
            role="alertdialog"
            aria-modal="true"
            onClick={() => setShowDeleteSupConfirm(false)}
          >
            <div
            ...
            >
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Are you sure you want to delete this supporter? This action
                cannot be undone.
              </p>
      ...
      {showDeleteConConfirm &&
        createPortal(
          <div
            className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm"
            role="alertdialog"
            aria-modal="true"
            onClick={() => setShowDeleteConConfirm(false)}
          >
            <div
              className="relative mx-4 w-full max-w-md rounded-2xl bg-white dark:bg-gray-900 p-6 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
